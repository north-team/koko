...
func NewDB2Connection(ops ...SqlOption) (*DB2Conn, error) {
    args := &sqlOption{
    ...
    return &DB2Conn{options: args, LocalCommand: lCmd}, nil
}
...
func startDB2Command(opt *sqlOption) (lcmd *localcommand.LocalCommand, err error) {
    initOnceLinuxDB2ShellFile()
    if db2ShellPath != "" {
        if lcmd, err = startDB2NameSpaceCommand(opt); err == nil {
            if lcmd, err = tryManualLoginDB2Server(opt, lcmd); err == nil {
                return lcmd, nil
            }
        }
    }
    if lcmd, err = startDB2NormalCommand(opt); err != nil {
        return nil, err
    }
    return tryManualLoginDB2Server(opt, lcmd)
}
...
func tryManualLoginDB2Server(opt *sqlOption, lcmd *localcommand.LocalCommand) (*localcommand.LocalCommand, error) {
    var (
        err error
        cmd    string
    ...
    cmd = fmt.Sprintf("connect to %s user %s using %s\n", opt.DBName, opt.Username, opt.Password)
    myBuffer.WriteString(cmd)
    _, err = lcmd.Write(myBuffer.Bytes())

    if err != nil {
        _ = lcmd.Close()
        logger.Errorf("DB2 local pty fd read err: %s", err)
        return lcmd, err
    }
